 ...
 */
package py.com.isproject.rodate.service;
...
import py.com.isproject.rodate.infra.model.Filter;
import py.com.isproject.rodate.infra.model.SortOrder;
...
public class CarService implements Serializable {
	...
	public List<Car> listByModel(String model) {
		return allCars.stream().filter(c -> c.getModel().equalsIgnoreCase(model)).collect(Collectors.toList());
	...
	public List<Car> paginate(Filter<Car> filter) {
		...
		List<Car> pagedList = allCars.stream().filter(predicates.stream().reduce(Predicate::or).orElse(t -> true))
				.collect(Collectors.toList());
	...
	private List<Predicate<Car>> configFilter(Filter<Car> filter) {
		...
		if (filter.hasParam("minPrice") && filter.hasParam("maxPrice")) {
			Predicate<Car> minMaxPricePredicate = (
					Car c) -> c.getPrice() >= Double.valueOf((String) filter.getParam("minPrice"))
							&& c.getPrice() <= Double.valueOf((String) filter.getParam("maxPrice"));
			predicates.add(minMaxPricePredicate);
		} else if (filter.hasParam("minPrice")) {
			Predicate<Car> minPricePredicate = (
					Car c) -> c.getPrice() >= Double.valueOf((String) filter.getParam("minPrice"));
			predicates.add(minPricePredicate);
		} else if (filter.hasParam("maxPrice")) {
			Predicate<Car> maxPricePredicate = (
					Car c) -> c.getPrice() <= Double.valueOf((String) filter.getParam("maxPrice"));
		...
		if (has(filter.getEntity())) {
			...
			if (has(filterEntity.getModel())) {
				Predicate<Car> modelPredicate = (Car c) -> c.getModel().toLowerCase()
						.contains(filterEntity.getModel().toLowerCase());
			...
			if (has(filterEntity.getName())) {
				Predicate<Car> namePredicate = (Car c) -> c.getName().toLowerCase()
						.contains(filterEntity.getName().toLowerCase());
	...
	public List<String> getModels(String query) {
		return allCars.stream().filter(c -> c.getModel().toLowerCase().contains(query.toLowerCase())).map(Car::getModel)
				.collect(Collectors.toList());
	}

	public void insert(Car car) {
		validate(car);
		car.setId(allCars.stream().mapToInt(c -> c.getId()).max().getAsInt() + 1);
		allCars.add(car);
	}
	...
	public void validate(Car car) {
		...
		if (allCars.stream().filter(c -> c.getName().equalsIgnoreCase(car.getName()) && c.getId() != c.getId())
				.count() > 0) {
